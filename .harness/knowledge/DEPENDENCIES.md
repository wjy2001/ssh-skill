...
- **所属层**：Service（`internal/ssh`、`internal/vault`）
- **升级策略**：人工审核
- **风险点**：当前锁定 **v0.17.0**（约 2023，对应 Go 1.18 最低要求）。**已知过时**（审查 F-003）：后续小版本含 SSH/加密安全修复；升级到 v0.21.0+ 会要求 Go 1.20，v0.31.0+ 要求 Go 1.22，v0.53.0+ 要求 Go 1.25。在未抬高 go.mod 最低版本前保持锁定；API 稳定性高，breaking change 罕见。**本文件仅记录风险，不修改 go.mod。**
- **替代方案**：无（Go 生态唯一成熟 SSH 和加密实现）
- **验证方式**：`go test ./internal/ssh/...` + `go test ./internal/vault/...`

### `github.com/pkg/sftp`
- **用途**：SFTP 文件上传/下载
- **所属层**：Service（`internal/ssh`）
- **升级策略**：人工审核
...
- **验证方式**：`go version` + `go build ./cmd/ssh-skill/`
