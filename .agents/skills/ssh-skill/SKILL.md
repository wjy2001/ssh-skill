...
description: |
  ...
  Bash, Codex's permission dialog gates approval; the bare ssh-skill binary has no built-in approval gate.
---
...
1. **Approval required (skill context)**: When this skill or controlled Bash invokes `${CLAUDE_SKILL_DIR}/bin/ssh-skill`, Codex's permission dialog gates approval. Running the bare `ssh-skill` binary outside Codex has no approval gate.
