...
- **Git Bash compatibility**: The bundled launcher preserves remote POSIX paths in `--remote` and `--command` when it invokes the native Windows binary, while retaining normal conversion for local `--local /c/...` paths.
